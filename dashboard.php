...
if (!$id = optional_param('id', null, PARAM_INT)) {
    foreach (dashboard::get_records() as $dashboard) {
        if ($dashboard->has_access($USER)) {
            redirect(new moodle_url('/local/dash/dashboard.php', ['id' => $dashboard->get('id')]));
        }
    }
...
$PAGE->set_url(new moodle_url('/local/dash/dashboard.php', [
    'id' => $dashboard->get('id')
]));
...
$PAGE->set_pagetype('local-dash-dashboard-' . $regionname);
