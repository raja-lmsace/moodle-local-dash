...
class dashboard_table extends \table_sql {
    ...
    public function col_name($data) {
        return \html_writer::link(new \moodle_url('/local/dash/dashboard.php', ['id' => $data->id]), $data->name);
    }
     ...
     * @return string
     */
    public function col_permissions($data) {
    ...
    public function col_actions($data) {
        ...
        return $OUTPUT->single_button(
                new \moodle_url('/local/dash/dashboards.php', ['action' => 'edit', 'id' => $data->id]),
                get_string('edit', 'block_dash'), 'get') .
            $OUTPUT->single_button(
                new \moodle_url('/local/dash/dashboards.php', ['action' => 'delete', 'id' => $data->id]),
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        $sql = 'SELECT * FROM {local_dash_dashboard} d ' . $wsql;
        ...
        if ($pagesize != -1) {
            $countsql = 'SELECT COUNT(DISTINCT d.id) FROM {local_dash_dashboard} d ' . $wsql;
