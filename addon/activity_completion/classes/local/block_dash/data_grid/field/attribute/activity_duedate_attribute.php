...
class activity_duedate_attribute extends abstract_field_attribute {
    ...
    public function transform_data($data, \stdClass $record) {
        ...
        if (dashaddon_activity_completion_is_timetable_installed()) {
            $duedate = $this->get_mod_user_duedate($cm, $userid) ?
                userdate($this->get_mod_user_duedate($cm, $userid), get_string('strftimedatefullshort'))
                : $duedate;
        }
        return $duedate;
    }
     ...
     */
    public function get_mod_user_duedate($mod, $userid) {
        global $DB;
