...
/**
 * Dashaddon calendar events data widget helper, process the event raw data to attribute.
 * Using more attributes, uses the widget attribute as base and uses the callback to this helper from the events_table.
 *
 * This way helps to prevent some additional DB queries. Reduce the events based attributes.
 */
class events {
    ...
    public function event_type() {
        ...
        } else if ($row->ce_courseid AND ($this->rowdata->eventtype == 'course' OR ($this->rowdata->modulename && $this->rowdata->instance))) {
            $type = 'course';
    ...
    public function get_description() {
        ...
        if (isset($this->event->description)) {
            ...
            $summary = file_rewrite_pluginfile_urls($this->data, 'pluginfile.php', $context, 'calendar', 'event_description', $this->rowdata->id);
